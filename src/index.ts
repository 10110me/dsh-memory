...
interface HttpResponseLike {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: { id: { type: 'string', required: true, description: 'Memory id to delete.' } },
    output: jsonOut(),
    execute: async function (args) {
      await ensureLoaded()
      const before = memories.length
      memories = memories.filter(function (m) { return m.id !== args.id })
      if (memories.length !== before) { kwIndex = null; await persistStore() }
      return { ok: true, deleted: before - memories.length }
    },
  }))
